...
object Data {
  val DATA = Array((0.0, 0.0), (1.0, 1.0), (2.0, 2.0), (3.0, 3.0), (4.0, 4.0))
  val DATA2 = Array((0.0, 0.0), (10.0, 1.0), (20.0, 0.5), (30.0, 1.5), (40.0, 1.0), (50.0, 2.5), (60.0, 3.0))
}

case class SinSignal(interval: Double, period: Double, scale: Double) extends Iterator[(Double, Double)] {
  var x: Double = 0.0

  override def hasNext: Boolean = true

  override def next(): (Double, Double) = {
    val point = (x, math.sin(x * 1.0 / period) * scale)
    x += interval
    point
  }
}
...
case class App(
    signal1: SinSignal,
    var data1: Array[(Double, Double)],
    signal2: SinSignal,
    var data2: Array[(Double, Double)],
...
object App {
  def apply(): App = {
    ...
    App(signal1, data1, signal2, data2, window = (0.0, 20.0))
  }
...
object Main {
  ...
  def ui(f: Frame, app: App): Unit = {
    ...
    val x_labels = Array(
      Span.styled(app.window._1.toString, Bold),
      Span.raw(((app.window._1 + app.window._2) / 2.0).toString),
      Span.styled(app.window._2.toString, Bold)
    )
