...
object Main {
  ...
  def ui(f: Frame, app: App): Unit = {
      ...
      val barchart2 = BarChart(
        block = Some(Block(title = Some(Spans.from("Data2")), borders = Borders.ALL)),
        data = app.data,
        bar_width = 5,
        bar_gap = 3,
      ...
      val barchart3 = BarChart(
        block = Some(Block(title = Some(Spans.from("Data3")), borders = Borders.ALL)),
        data = app.data,
