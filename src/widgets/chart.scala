...
case class Chart(
    ...
    /// The horizontal axis
    x_axis: Axis = Axis.default,
    /// The vertical axis
    y_axis: Axis = Axis.default,
  ...
  def layout(area: Rect): ChartLayout = {
    var layout = ChartLayout.default
  ...
  def render_x_labels(buf: Buffer, layout: ChartLayout, chart_area: Rect, graph_area: Rect): Unit = {
  ...
  def render_y_labels(buf: Buffer, layout: ChartLayout, chart_area: Rect, graph_area: Rect): Unit = {
  ...
  override def render(area: Rect, buf: Buffer): Unit = {
    ...
    layout.legend_area match {
      ...
      case Some(legend_area) =>
        ...
        Block(borders = Borders.ALL).render(legend_area, buf)
